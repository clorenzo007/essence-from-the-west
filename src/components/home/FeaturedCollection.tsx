...
export function FeaturedCollection({ products, genusHighlights = [] }: FeaturedCollectionProps) {
      ...
      <div className="ro-container">
        <div className="mb-16 flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          {showProducts ? (
            ...
            <SectionHeading
              label="Selección"
              title="Explorá por género"
              description="Orquídeas de colección — Cattleya, Vanda, Phalaenopsis y más — cada una con su propio carácter y su flor."
            />
          )}
        ...
        {showProducts ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {photoHighlights.slice(0, 8).map((sheet) => (
              <Link key={sheet.id} href={`/care/${sheet.slug}`} className="group block">
